...
int main(void){
    ...
    KDTree simplekd(triangles);
    simplekd.Build(13, 1);

    int num_rays = 2'000'000;
    float ray_bound = 3.0f;
    std::vector<Ray> rays;
    for(int i = 0; i < num_rays; i++) {
        Ray ray;
        ray.origin = glm::sphericalRand(ray_bound);
        ray.direction = glm::sphericalRand(ray_bound);
    ...
    // if(accelerated) {
        ...
        // hits = 0;
        for(Ray& ray : rays) {
            if(simplekd.RayIntersect(ray, &intersection)) {
                hits++;
            }
        }
    // } else {
